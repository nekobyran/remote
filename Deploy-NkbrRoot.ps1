...
function Invoke-Validate {
  ...
  if (([regex]::Matches($index, '<article class="release-card">')).Count -ne 3) {
    throw '根发布区必须展示 LanzouPlus、Kacha 与 Easy Dream Skin。'
  }
  ...
  if ($index -notmatch '(?s)<article class="release-card">.*?<h3>Easy Dream Skin</h3>.*?<dt>版本</dt><dd>1\.5\.5</dd>.*?href="https://github\.com/nekobyran/easy-dream-skin".*?href="https://github\.com/nekobyran/easy-dream-skin/blob/main/LICENSE".*?href="https://github\.com/nekobyran/easy-dream-skin/releases/tag/v1\.5\.5".*?</article>') {
    throw 'Easy Dream Skin 必须以 v1.5.5、公开仓库和 MIT 许可信息出现在根发布区。'
  }
  ...
  $easyDreamSkinIconPath = Join-Path $project 'easy-dream-skin-icon.webp'
  $easyDreamSkinIconHash = (Get-FileHash -LiteralPath $easyDreamSkinIconPath -Algorithm SHA256).Hash
  if ($easyDreamSkinIconHash -ne 'F93C00BBF46984B26B7C2BA9184C2E6E8C11B02C8270CEA492590610D93CECDA') {
    throw 'Easy Dream Skin 图标不是已核验的软件图标。'
  }
  ...
  'validation=pass;main-releases=3;kacha=online;easy-dream-skin=online;roadmap=8;lanzouyou-after-gamelauncher=true;codexmax=development;flclashplusplus-local-only=true;private-assets=0;sponsor=local'
}
